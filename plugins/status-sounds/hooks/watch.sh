#!/usr/bin/env bash
# Sound the alert the moment Claude starts waiting, instead of six seconds later.
#
...
# +6020 ms for the event. So this watches that file and calls play.sh on the
# transition, four times a second.
#
# The event stays wired up as a fallback: if this process never started or has
# died, the six-second notification is still better than silence. `marker` is
# what tells the two apart — it exists for exactly as long as one waiting
# episode, and play.sh bails when it finds it.
#
...
#
#   STATUS_SOUNDS_ALERT_DELAY    seconds to stay blocked before the sound, 0 by
#                                default. Raise it towards Claude Code's own 6 s
#                                if prompts you answer at once feel too chatty.
#   STATUS_SOUNDS_POLL_INTERVAL  how often to look, 0.25 s by default. It caps
#                                how late the sound can be.
interval=${STATUS_SOUNDS_POLL_INTERVAL:-0.25}
alert_delay=${STATUS_SOUNDS_ALERT_DELAY:-0}
...
while :; do
  ...
  if [[ $content =~ $waiting_re ]]; then
    # The marker goes up as the episode starts, not as the sound plays: it says
    # "a watcher owns this one", so the fallback event stays quiet even when the
    # configured delay puts our sound after it.
    ((held == 0)) && : >"$marker"
    held=$((held + 1))
    if ((fired == 0 && held > delay_ticks)); then
      fired=1
      printf '{"hook_event_name":"Notification","session_id":"%s"}' "$session_id" |
        STATUS_SOUNDS_FROM_WATCHER=1 bash "$here/play.sh" >/dev/null 2>&1 &
    fi
  elif ((held > 0)); then
    held=0
    fired=0
